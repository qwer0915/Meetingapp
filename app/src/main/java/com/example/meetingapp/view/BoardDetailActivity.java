    ...
    public class BoardDetailActivity extends BaseActivity {
        ...
        private void initViews(int questionId){
            ...
            answerAdapter = new AnswerAdapter(this, api, questionId, (answer, pos) -> {
                Intent intent = new Intent(this, AnswerFormActivity.class);
                intent.putExtra("mode", "edit");
                intent.putExtra("answerId", answer.id);
                intent.putExtra("content", answer.content);
                editLauncher.launch(intent);
            });
            Log.d("YourFragmentOrActivity", "Adapter 생성시 questionId: " + questionId);
                        ...
                        if (answerObj instanceof List<?>) {
                            ...
                            for (Object o : rawList) {
                                if (o instanceof Map) {
                                    ...
                                    try {
                                        int answerId = ((Number)ans.get("id")).intValue();
                                        String author = String.valueOf(ans.get("author"));
                                        int questionId = ((Number)ans.get("question_id")).intValue();
                                        String content = String.valueOf(ans.get("content"));
                                        String date = String.valueOf(ans.get("create_date"));
                                        answers.add(new AnswerItem(answerId, author, questionId, date, content));
                                    } catch (Exception e) {
                                        // 파싱 실패한 항목은 무시
                                        Log.e("BoardDetail", "답변 파싱 오류: " + ans, e);
                                    }
                                }
                            }
